/**
 * Modèles SVG pour les exercices
 */
const EX_TEMPLATES = [
    {
        keywords: ["pompe", "push", "gainage", "planche"],
    ...
    {
        keywords: ["traction", "pull", "muscle up"],
        template: (strokeColor) => `
            <!-- Barre -->
    ...
    {
        keywords: ["squat", "fente", "chaise", "leg"],
        template: (strokeColor) => `
    ...
    {
        keywords: ["course", "run", "sprint", "jog"],
        template: (strokeColor) => `
...
function getDefaultSvgContent(strokeColor) {
    return `
...
function getExImage(ex) {
    ...
    const matchedTemplate = EX_TEMPLATES.find(t =>
        t.keywords.some(keyword => name.includes(keyword))
    );

    const svgContent = matchedTemplate
        ? matchedTemplate.template(strokeColor)
        : getDefaultSvgContent(strokeColor);
